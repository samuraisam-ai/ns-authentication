...
export async function POST(req: Request) {
  ...
  console.log("[checkins-submit-and-coach] session created", { sessionId, userId: user.id, taskId: task.id });
  ...
  if (userMessageError) {
    console.error("[checkins-submit-and-coach] Failed to insert user message:", userMessageError);
    return NextResponse.json(
  ...
  console.log("[checkins-submit-and-coach] inserted chat_message", { role: "user", sessionId });
  ...
  console.log("[checkins-submit-and-coach] inserted chat_message", { role: "assistant", sessionId, type: "placeholder" });
  ...
  const sendResult = await logChatEvent({
    userId: user.id,
    direction: "send",
  ...
  if (!sendResult.ok) {
    console.error("[checkins-submit-and-coach] chat_events send log failed", sendResult.error);
  }
  ...
  try {
    ...
    console.log("[checkins-submit-and-coach] n8n response", { status: webhookResponse.status, ok: webhookResponse.ok });
  ...
  console.log("[checkins-submit-and-coach] inserted chat_message", { role: "assistant", sessionId, type: "coaching_reply" });

  const receiveResult = await logChatEvent({
    userId: user.id,
    direction: "receive",
  ...
  if (!receiveResult.ok) {
    console.error("[checkins-submit-and-coach] chat_events receive log failed", receiveResult.error);
  }
